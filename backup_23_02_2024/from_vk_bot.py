...
def send_posts(user_id, post_args):
    ...
    try:
        ...
        if img_url_list_len == 0:
            ...
            bot.send_photo(user_id, img_url_list[0], default_caption, reply_markup=markup,
                           parse_mode='HTML')
        else:
        ...
        print(e)
        with open(f"error_log_sending{post_id}.txt", "w") as f:
            f.write(str(post_args)+"\n\n"+str(e))
        ...
        try:
            if post_data:
                for single_post in post_data:
                    send_posts(user_id, single_post)
